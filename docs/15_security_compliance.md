...
- `POST /api/v1/webhooks/whatsapp/{tenant_id}` (signature verification and duplicate handling)
- `POST /api/v1/webhooks/ecommerce/{tenant_id}` (tenant-scoped integration ingestion)
- `WS /api/v1/ws/chat/{tenant_id}` (tenant/session-scoped realtime channel)
- `POST /api/v1/widget/messages` (authenticated/authorized message ingress path)
